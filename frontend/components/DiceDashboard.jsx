  ...
  return (
    <>
      <div className="mb-2 text-center">
        {address ? (
          <>
            <div className="text-2xl mr-6">Dice Balance</div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              {/* Changed to single column on small screens */}
              <div className="text-right">
                {/* Adjusted alignment to right */}
                <p>0: {diceBalance[0]}</p>
                <p>1: {diceBalance[1]}</p>
                <p>2: {diceBalance[2]}</p>
                <p>3: {diceBalance[3]}</p>
              ...
              <div className="flex flex-col justify-end items-center">
                {" "}
                {/* Adjusted alignment to bottom */}
                {/* Moved buttons to second column */}
                <button
                  onClick={mintDice}
                  className="p-2 bg-white text-black rounded-lg my-2 active:bg-gray-200"
                >
                  {txnLoadingMinting ? "Minting Dice..." : "🎲 Mint Dice"}
                </button>
                <button
                  onClick={burnDice}
                  className="p-2 bg-orange-200 text-black rounded-lg my-2 active:bg-gray-200"
                >
          ...
          <div className="mr-6">Connect to see dice balance</div>
        )}
      </div>
    </>
  );
